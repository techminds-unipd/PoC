...
export class WorkflowsService {
    ...
    async execute(id: string) {
        ...
        const workflowAndTokens = {workflow: {name: workflow.name, nodes: workflow.nodes, edges: workflow.edges}, googleToken: user.token}
        this.httpService.post('http://127.0.0.1:5000/execute', workflowAndTokens)
            .subscribe({
